    ...
    class ReadTextFilesImpl : ISource
    {
             ...
             using (StreamReader rdr = new StreamReader(path))
                {
                    while ((line = rdr.ReadLine()) != null)
                    {
                        ...
                        else if (line.ToLower().Contains(ParseRules.PC)) ProcessEquipment(line, sqlModel);
        ...
        private void ReadHistoricalData(string line, List<HistoricalData> historicalData, List<string> headers, string fileName)
        {
            try
            {
                ...
                // Approximatelly -> not smartest way to test. TTTTTTT please find a solution :-)
                ...
                //if (headers.Count - 5 > splittedLine.Count() || line.Contains(ParseRules.IgnoredStrings[0]))
                //{
                //    return;
                //}
